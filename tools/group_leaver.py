    ...
    try:
        for cookie in cookies:
            async with aiohttp.ClientSession(cookies={".ROBLOSECURITY": cookie['cookie']}) as session:
                groups = await get_groups(session, cookie['id'])
                self.display_theme(1)
                if groups is not None and len(groups) >= 1:
                    cprint.info(f"Gathered {len(groups)} groups for {cookie['name']}")
                    ...
                    if choice in ["yes", "y"]:
                        ...
                        tasks = [asyncio.create_task(leave_groups(session, group, cookie['id'], whitelisted_ids)) for group in groups]
                    ...
                    if choice in ["no", "n"]:
                        for group in groups:
                            ...
                            if choice in ["yes", "y"]:
                                await leave_group(session, group, cookie['id'], cookie['cookie'])
                                continue
